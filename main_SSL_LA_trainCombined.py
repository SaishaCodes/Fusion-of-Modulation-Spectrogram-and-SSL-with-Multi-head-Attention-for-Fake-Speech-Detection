...
import yaml
from data_utils_SSL import genSpoof_list_mlaad,Dataset_mlaad_devNeval, Dataset_mlaad_train
...
if __name__ == '__main__':
    ...
    if not os.path.exists('models_combined_2'):
        os.mkdir('models_combined_2')
    ...
    model_save_path = os.path.join('models_combined_2', model_tag)
    ...
    #evaluation 
    if args.eval:
        file_eval = genSpoof_list_mlaad( dir_meta =  os.path.join(args.protocols_path+"combined_protocol/combined_eval.txt"),is_train=False,is_eval=True)
    ...
    d_label_trn,file_train = genSpoof_list_mlaad( dir_meta =  os.path.join(args.protocols_path+"combined_protocol/combined_train.txt"),is_train=True,is_eval=False)
    
    print('no. of training trials',len(file_train))
    
    train_set=Dataset_mlaad_train(args,list_IDs = file_train,labels = d_label_trn,base_dir = trn_database_path,algo=args.algo)
    ...
    d_label_dev,file_dev = genSpoof_list_mlaad( dir_meta =  os.path.join(args.protocols_path+"combined_protocol/combined_dev.txt"),is_train=False,is_eval=False)
    
    print('no. of validation trials',len(file_dev))
    
    dev_set = Dataset_mlaad_train(args, list_IDs = file_dev,labels=d_label_dev, base_dir=dev_database_path, algo=args.algo)
