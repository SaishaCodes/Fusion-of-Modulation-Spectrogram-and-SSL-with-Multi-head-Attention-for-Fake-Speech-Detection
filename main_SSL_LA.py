...
def produce_evaluation_file(dataset, model, device, save_path):
    data_loader = DataLoader(dataset, batch_size=20, shuffle=False, drop_last=False)
    num_correct = 0.0
    num_total = 0.0
    model.eval()
...
if __name__ == '__main__':
    ...
    if not os.path.exists('s_models'):
        os.mkdir('s_models')
    ...
    model_save_path = os.path.join('s_models', model_tag)
